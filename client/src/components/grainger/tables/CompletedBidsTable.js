...
class CompletedBidsTable extends Component {
  ...
  getCompletedList (activePage) {
    ...
    let url = ('/api/grainger/completed_bids/?page=' + activePage);
    fetch(url)
    .then(res => res.json())
    .then(data => {
  ...
  getCount () {
    fetch('/api/grainger/completed_bids/count')
    .then(res => res.json())
    .then(data => {
  ...
  getProfit () {
    fetch('/api/grainger/completed_bids/profit')
    .then(res => res.json())
    .then(data => {
  ...
  getMargin () {
    fetch('/api/grainger/completed_bids/margin')
    .then(res => res.json())
    .then(data => {
          ...
          {this.state.loading &&<Dimmer active>
            ...
            {allCompleted.map(q =>
              <Table.Row key={q.id}>
                ...
                <Table.Cell><MainModal icon={true} id={q.id} header={'grainger'}/></Table.Cell>
              </Table.Row>
            )}
        </Table>
      </div>
